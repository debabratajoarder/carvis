            ...
            async: false,
           	success: function(respsText) {
				...
				if(respsText != "blank"){
					...
					alert("No Order Templete Exist With This Customer");
				}				
            ...
            <div id="collapseOne" class="accordion-body collapse in body">
                <div class="col-sm-12">
				    ...
				    <fieldset>
						...
						<input type="hidden" name="address_name" id="address_name" value="<?=$order->address_name?>" />
						<input type="hidden" name="run_id" id="run_id" value="<?=$order->run_id?>" />
						<input type="hidden" name="run_name" id="run_name" value="<?=$order->run_name?>" />
						<input type="hidden" name="gpVal" id="gpVal" value="<?=$order->customer['group_priority']?>" />

						<div class="form-group">
							<div class="input">
								<label>Order Date : <?=$order->order_date?> </label>
							</div>
						</div>
						...
						<div class="form-group">
							<div class="input">
								...
								<?php foreach($customers as $custKey=>$custval){?>
									<?php if($order->customer_id == $custKey){ ?>
