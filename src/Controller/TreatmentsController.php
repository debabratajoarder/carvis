...
class TreatmentsController extends AppController {
    ...
    public function treatmentdetail($slug = null, $datap = null) {
        ...
        $treatment = $this->Treatments->find()
                                      ->contain(['Categories', 'Medicines','Medicines.Pils' => ['sort' => ['Pils.title' => 'ASC']],'Medicines.Pils.Pilprices','TreatmentQuestions','TreatmentQuestions.Questions','TreatmentQuestions.Questions.QuestionCheckboxes'])
    ...
    public function cart() {
        ...
        if ($this->request->session()->check('Auth.Doctor')) {
            $this->Flash->error('Please Login as Patient.');
    ...
    /*
    public function treatmentprice($slug = null, $orderid = null) {
        ...
        $treatment = $this->Treatments->find()
                  ->hydrate(false)
                  ->where(['Treatments.slug' => $slug])
                  ->contain(['Categories' => function($q) { return $q->select(['Categories.id','Categories.name','Categories.slug']); }])
                  ->contain(['Medicines' => function($q) { return $q->select(); }])->toArray();
        
        
        
//        $questionlistarr = $this->MedicineQuestions->find();
//                        ->hydrate(false)
//                        ->where(['MedicineQuestions.mid' => $treatment[0]['Medicines'][0]['id']])
//                        ->contain(['Question' => function($q) { return $q->select(); }])->toArray();
//        
//                        
//        $questionlist = array();
//        $i = 1;
//        foreach($questionlistarr as $qla){
//            $questionlist[$i] = $qla['Questions'];
//            
//            if($qla['Questions']['answer_type'] != "y"){
//                $valconid = $qla['Questions']['id'];
//                $questionlist[$i]['checkboxlist'] = $this->QuestionCheckboxes->find('all')
...
//        }
        ...
        $medicine_list = $treatment[0]['Medicines'];
        ...
        //pr($questionlist); 
        //echo $order_id; pr($treatment);  exit;          
                  
        //$treatment = $this->paginate($this->Treatments,['limit' => 10, 'conditions' => ['Treatments.parent_id' => 0]]);
        $this->set(compact('treatment','questionlist','order_id','medicine'));
        $this->set('_serialize', ['treatment']);         
    ...
    */
    ...
    public function cart() {
        ...
        //$session = $this->request->session(); $session->write('amount',''); $session->write('mycart','');
        $session = $this->request->session();
        $precart = $session->read('mycart');
        $precartamt = $session->read('amount');
        //pr($precart); //exit;
        ...
        $ctreatment = array();
        $ci = 0;
        foreach($precart as $k=>$mycart){
            
            $treatment = $this->Treatments->get($mycart['tid'])->toArray(); 
            $medicine = $this->Medicines->get($mycart['mid'])->toArray();
            $pils = $this->Pils->get($mycart['pid'])->toArray();
            
            //pr($treatment); pr($medicine); pr($pils); exit;
                

            
            
            $cartData[$treatment['name']]['Detail'][$k]['Medicine']['title'] = $medicine['title'];
            $cartData[$treatment['name']]['Detail'][$k]['Medicine']['id'] = $medicine['id'];
            $cartData[$treatment['name']]['Detail'][$k]['Pils'] = $pils;
            $cartData[$treatment['name']]['Detail'][$k]['Data'] = $mycart;
            
            //$cartData[$mycart['tid']]['Treatment']['name'] = $treatment['name'];
            //$cartData[$mycart['tid']]['Treatment']['id'] = $treatment['id'];
            //$cartData[$mycart['tid']]['Treatment']['Medicine']['name'] = $medicine['title'];
            //$cartData[$mycart['tid']]['Treatment']['Medicine']['id'] = $medicine['id'];
            //$cartData[$mycart['tid']]['Treatment']['Medicine']['Pils'] = $pils;
        ...
        $this->set(compact('precart','cartData'));
        $this->set('_serialize', ['cartData']);         
