...
//
    ...
    public function addservice($id) {
       ...
       // pr($stagname);exit;
        $this->set(compact('service','stname','sfname','stagname'));
        $this->set('_serialize', ['service']);
    }
...
//                                                ->send($mail_body);                
     ...
     public function editservice($eid = null,$id = null) {
           ...
           /* if($this->request->data['address'] == ""){
            ...
            if($flag){
                ...
                if ($this->Services->save($user)) {
                   ...
                   $this->ServiceProviderTypes->deleteAll(['service_id'=>$eid]);
                   $type_id= $this->request->data['service_type_id'];
                    $min_price= $this->request->data['min_price'];
                    $max_price= $this->request->data['max_price'];
                    ...
                    $this->ServiceProviderModels->deleteAll(['service_id'=>$eid]);
        ...
        $stagname=$this->Models->find('all', array('conditions' => array('Models.make_id IN' =>$stagid)))->contain(['Makes']);
         ...
         $sptypename=$this->ServiceProviderTypes->find()->where(['service_id'=>$eid])->contain(['ServiceTypes'])->toArray();
         //pr($sptypename);exit;
        foreach ($sptypename as $value) {
            $sptypenames[] = $value['type_id'];
        }
        ...
        //pr($spfname);
        $sptagname=$this->ServiceProviderModels->find()->select(['model_id'])->where(['service_id'=>$eid])->toArray();
        foreach ($sptagname as $value) {
            $sptagnames[] = $value['model_id'];
        }
        //$sptagname=$this->ServiceProviderTags->find('all', array('conditions' => array('service_id' =>$stagid),'fields'=>'tag_id'));
       // pr($sptagname);
       
        $this->set(compact('user','stname','sfname','stagname','spfname','sptagnames','spfnames','sptypename','sptypenames'));
        $this->set('_serialize', ['user']);
    }
...
//            }
            ...
            if($flag){
               ...
               // print_r($new_st);exit;
                ...
                $this->Flash->success('Service provider added successfully.', ['key' => 'success']);
